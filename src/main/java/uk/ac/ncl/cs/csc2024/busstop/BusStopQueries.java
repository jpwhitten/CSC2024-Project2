...
public class BusStopQueries {
    ...
    public static Session insert(final Map<String, String> row, final Session session) {
    	...
    	bus.setDescription(row.get("description"));
    	bus.setId(Integer.parseInt(row.get("id")));
    	session.save(bus);    	
    ...
    public static ExampleQuery selectMaxId() {
        return new ExampleQuery() {
            ...
            public Criteria getCriteria(Session session) {            	
            	...
            	DetachedCriteria getMax = DetachedCriteria.forClass(BusStop.class)
            			.setProjection(Projections.max("id"));
            	
            	criteria.add(Subqueries.propertyIn("id", getMax));
